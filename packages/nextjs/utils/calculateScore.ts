...
export const calculateScore = (userProfile: UserScore): Score => {
  const transactionPoints = calculateTransactionPoints(userProfile.transactions);
  const accountAgePoints = calculateAccountAgePoints(userProfile.accountAgeMonths);
  const protocolsUsedPoints = calculateProtocolsUsedPoints(userProfile.protocolsUsed);
  const monthsInteractingPoints = calculateMonthsInteractingPoints(userProfile.monthsInteracting);
  const grassBalancePoints = calculateGrassBalancePoints(userProfile.grassBalance);

  const totalScore = transactionPoints + accountAgePoints + protocolsUsedPoints + monthsInteractingPoints + grassBalancePoints;
